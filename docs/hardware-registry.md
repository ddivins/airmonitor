...
The sensor service reads `AIRMONITOR_HARDWARE_ID` from `/etc/airmonitor/sgx-voc.env`. When `AIRMONITOR_PORT=auto`, it resolves that hardware id at startup.
...
An explicit `AIRMONITOR_PORT` in `/etc/airmonitor/sgx-voc.env` always bypasses registry discovery. Set it to `auto` to use the registry.
