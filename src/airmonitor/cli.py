...
DEFAULT_INSTALL_CONF = "/etc/airmonitor/install.conf"
...
def _read_repo_dir(install_conf: str | Path = DEFAULT_INSTALL_CONF) -> str | None:
    path = Path(install_conf)
    if not path.exists():
        return None
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if key.strip() == "REPO_DIR":
            return value.strip() or None
    return None


def resolve_repo_dir() -> str:
    """Locate the Git checkout tools/update.sh and tools/install.sh live in.

    The installed `airmonitor` command runs from /opt/airmonitor/venv, a
    different path from wherever the checkout was cloned to, so this can't
    just be derived from this file's own location the way the shell scripts
    derive theirs.
    """

    repo_dir = os.environ.get("AIRMONITOR_REPO_DIR") or _read_repo_dir(DEFAULT_INSTALL_CONF)
    if not repo_dir:
        raise SystemExit(
            "Could not determine the AirMonitor Git checkout location.\n"
            f"Add REPO_DIR=/path/to/airmonitor to {DEFAULT_INSTALL_CONF} "
            "(tools/install.sh writes this automatically on new installs), "
            "or set AIRMONITOR_REPO_DIR."
        )
...
def update(args: argparse.Namespace) -> int:
    repo_dir = resolve_repo_dir()
    script = Path(repo_dir) / "tools" / "update.sh"
    if args.dry_run:
        print("AirMonitor update plan (dry run; pass --no-dry-run to execute)")
        print(f"  repo: {repo_dir}")
        print(f"  command: git -C {repo_dir} pull --ff-only && bash {script}")
        return 0
    # Deliberately not run as root: tools/update.sh escalates internally only
    # where needed, and `git pull` must run as whichever user actually has
    # the checkout's Git/SSH credentials, not root.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    install_parser.add_argument(
        ...
        help="show what would run without executing tools/install.sh (default: on)",
    )
    # Extra tools/install.sh flags (--full, --non-interactive, etc.) are
    # captured via parse_known_args in main() rather than a REMAINDER
    # positional here: argparse's REMAINDER fails to capture leading `--`
    # tokens when nothing precedes them, which is exactly this command's
    # most natural invocation shape.

    update_parser = subparsers.add_parser("update", help="git pull and run tools/update.sh from the located checkout")
